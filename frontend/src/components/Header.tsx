...
export default function Header({ onCartClick }: HeaderProps) {
  ...
  return (
    <header className="bg-black/95 backdrop-blur-md shadow-sm border-b border-gold/20 sticky top-0 z-50">
        ...
        <div className="flex justify-between items-center h-16 sm:h-20">
          ...
          <Link to="/" className="flex items-center space-x-3 group min-w-0">
            <div className="flex flex-col items-center leading-none flex-shrink-0">
              <span className="font-bodoni text-2xl sm:text-4xl font-normal bg-gradient-to-br from-gold-light via-gold to-gold-dark bg-clip-text text-transparent" style={{letterSpacing: '-2px'}}>
                E
              </span>
            </div>
            <div className="flex flex-col leading-none min-w-0">
              <span className="font-prata text-base sm:text-xl text-white uppercase tracking-[6px] sm:tracking-[10px] truncate">
                EGOS
              </span>
              <span className="font-bodoni italic text-[9px] sm:text-[10px] tracking-[3px] text-gold opacity-80 uppercase mt-0.5 hidden sm:block">
                Wear Your Truth
              </span>
            </div>
          </Link>
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            ...
            {/* Favoritos */}
            <Link to={isAuthenticated ? '/favorites' : '/login'} className="relative p-2 sm:p-2.5 text-gray-300 hover:text-gold hover:bg-white/10 rounded-xl transition-all">
            ...
            {isAuthenticated ? (
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  className="flex items-center space-x-2 pl-1 pr-2 py-1 rounded-xl hover:bg-white/10 transition-all group"
                >
                  {/* Avatar con inicial */}
                  ...
                  <div className="hidden md:block text-left">
                    <p className="text-xs font-semibold text-gray-200 leading-tight">{user?.nombre?.split(' ')[0]}</p>
                ...
                {/* Dropdown */}
                {showUserMenu && (
                  <div className="absolute right-0 top-14 bg-gray-900 rounded-2xl shadow-2xl border border-gold/20 p-2 z-50 min-w-[220px] animate-fadeIn">
                    {/* Info usuario */}
                    <div className="px-3 py-2 mb-1 border-b border-white/10">
                      <p className="font-semibold text-gray-100 text-sm">{user?.nombre}</p>
              ...
              <Link to="/login" className="flex items-center space-x-1.5 bg-gold text-black px-2.5 sm:px-4 py-2 text-xs sm:text-sm rounded-xl hover:bg-gold-light transition-all shadow-sm flex-shrink-0 font-semibold">
...
function MenuItem({ to, icon, label, onClick }: { to: string; icon: string; label: string; onClick: () => void }) {
  return (
    <Link
      to={to}
      onClick={onClick}
      className="flex items-center space-x-2.5 px-3 py-2 text-sm text-gray-300 hover:bg-white/10 hover:text-gold rounded-xl transition-colors"
    >
