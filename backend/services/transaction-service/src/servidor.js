...
aplicacion.get('/api/eventos/pedidos', (req, res) => {
  ...
  console.log(`📡 SSE conectado: usuario ${usuarioId}`);
...
function emitirSSE(usuarioId, evento, datos) {
  ...
  clientes.forEach(res => { try { res.write(msg); } catch(e) {} });
  console.log(`📡 SSE emitido a usuario ${usuarioId}`);
}
...
aplicacion.post('/api/pagos/epayco/widget', autenticacion, async (req, res) => {
  try {
    ...
    if (!epayco.estaConfigurado()) {
      ...
      const condicionPedido = 'SELECT * FROM pedido WHERE id = $1 AND usuario_id = $2';
    const paramsPedido = [pedido_id, parseInt(req.usuario.id)];
