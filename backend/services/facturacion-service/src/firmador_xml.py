...
CORRECCIONES vs versión anterior:
  - DigestMethod en todas las References → SHA256 (igual que ejemplo Generica.xml)
  - DigestMethod en CertDigest           → SHA256
  - URL política de firma                → v1/politicadefirmav2.pdf  (no v2/)
  - SigPolicyHash DigestValue            → valor correcto SHA256
"""
import base64
import hashlib
import uuid
...
# URL y hash SHA256 de la política de firma DIAN (versión 1, no v2)
POLITICA_URL    = "https://facturaelectronica.dian.gov.co/politicadefirma/v1/politicadefirmav2.pdf"
POLITICA_DIGEST = "dMoMvtcG5aIzgYo0tIsSQeVJBDnUnfSOfBpxXrmor0Y="   # SHA256 base64

# Algoritmos — SHA256 igual que los ejemplos oficiales DIAN
...
class FirmadorXML:
    def __init__(self, p12_path: str = None, p12_password: str = None):
        self.p12_path     = p12_path     or os.getenv("DIAN_P12_PATH",     "/app/certs/certificado.pfx")
        ...
        self.cert_der     = None
        self.cert_b64     = None
        self.cert_digest  = None   # SHA256 base64
    ...
    def cargar_certificado(self):
        ...
        try:
            ...
            # SHA256 del certificado DER (igual que ejemplo DIAN)
            self.cert_digest   = base64.b64encode(
                hashlib.sha256(self.cert_der).digest()
            ).decode()
            ...
            print(f"❌ Certificado no encontrado: {self.p12_path}")
            return False
    ...
    def firmar(self, xml_string: str) -> str:
        if not self._loaded:
            if not self.cargar_certificado():
                raise Exception("No se pudo cargar el certificado .p12/.pfx")

        tree = etree.fromstring(xml_string.encode("utf-8"))

        # Extraer fecha/hora del documento para SigningTime
        _date = re.search(r"<cbc:IssueDate>([^<]+)", xml_string)
        _time = re.search(r"<cbc:IssueTime>([^<]+)", xml_string)
        signing_time = (
            f"{_date.group(1)}T{_time.group(1)}"
            if _date and _time
            else datetime.now(COLOMBIA_TZ).strftime("%Y-%m-%dT%H:%M:%S-05:00")
        )
        ...
        # Reference 0 — documento completo con enveloped-signature
        ref0 = etree.SubElement(signed_info, f"{{{NS_DS}}}Reference",
                                Id=ref0_id, URI="")
        transforms = etree.SubElement(ref0, f"{{{NS_DS}}}Transforms")
        etree.SubElement(transforms, f"{{{NS_DS}}}Transform",
                         Algorithm=ALG_ENV_SIG)
        etree.SubElement(ref0, f"{{{NS_DS}}}DigestMethod", Algorithm=ALG_SHA256)
        etree.SubElement(ref0, f"{{{NS_DS}}}DigestValue")   # xmlsec lo calcula

        # Reference KeyInfo
        ref_ki = etree.SubElement(signed_info, f"{{{NS_DS}}}Reference",
                                  URI=f"#{keyinfo_id}")
        etree.SubElement(ref_ki, f"{{{NS_DS}}}DigestMethod", Algorithm=ALG_SHA256)
        ...
        etree.SubElement(ref_sp, f"{{{NS_DS}}}DigestMethod", Algorithm=ALG_SHA256)
        ...
        etree.SubElement(cd, f"{{{NS_DS}}}DigestMethod", Algorithm=ALG_SHA256)
        ...
        # SignaturePolicyIdentifier — URL v1 (no v2)
        ...
        etree.SubElement(spid_id, f"{{{NS_XADES}}}Identifier").text = POLITICA_URL
        etree.SubElement(spid_id, f"{{{NS_XADES}}}Description")
        sph = etree.SubElement(spid, f"{{{NS_XADES}}}SigPolicyHash")
        etree.SubElement(sph, f"{{{NS_DS}}}DigestMethod", Algorithm=ALG_SHA256)
        etree.SubElement(sph, f"{{{NS_DS}}}DigestValue").text = POLITICA_DIGEST
